...
def evaluation_and_show(model):
    with torch.set_grad_enabled(False):
        ...
        # 获取标签名称
        label_names = datasets.CIFAR10(root='data', train=False).classes

        # 显示前10张测试图像
        dataiter = iter(test_loader)  # 获取测试集数据的迭代器
        images, labels = dataiter.next()  # 使用 next 函数获取数据集中的一个 batch
        ...
        images = images * np.array([0.2023, 0.1994, 0.2010]) + np.array([0.4914, 0.4822, 0.4465])  # 还原图像
